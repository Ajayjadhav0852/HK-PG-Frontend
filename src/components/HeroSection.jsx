...
function BookVisitModal({ onClose }) {
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
    >
      <div style={{ background: 'white', borderRadius: 24, width: '100%', maxWidth: 420, boxShadow: '0 32px 80px rgba(0,0,0,0.35)', overflow: 'hidden', animation: 'modalIn 0.3s cubic-bezier(0.22,1,0.36,1)' }}>
        <div style={{ background: 'linear-gradient(135deg,#d63384,#c026d3)', padding: '24px 24px 20px', position: 'relative' }}>
          <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', width: 32, height: 32, borderRadius: '50%', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="22" height="22"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
            </div>
            <div>
              <h2 style={{ color: 'white', fontWeight: 800, fontSize: 18, margin: 0 }}>Book a Visit</h2>
        ...
        <div style={{ padding: '24px' }}>
          {done ? (
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="56" height="56" style={{ margin: '0 auto 12px', display: 'block' }}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
              <h3 style={{ fontWeight: 800, color: '#111827', fontSize: 18, margin: '0 0 8px' }}>Request Sent!</h3>
              <p style={{ color: '#6b7280', fontSize: 14, margin: '0 0 20px', lineHeight: 1.6 }}>The owner has been notified via WhatsApp. You'll receive a call shortly to schedule your visit.</p>
              <button onClick={onClose} style={{ width: '100%', padding: '12px', background: 'linear-gradient(135deg,#d63384,#c026d3)', color: 'white', border: 'none', borderRadius: 12, fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>Done</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>I am a *</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {VISITOR_TYPES.map(t => (
                    <button key={t.key} type="button" onClick={() => setForm(f => ({ ...f, type: t.key }))}
                      style={{ padding: '8px 16px', borderRadius: 100, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s ease', background: form.type === t.key ? 'linear-gradient(135deg,#d63384,#c026d3)' : 'white', color: form.type === t.key ? 'white' : '#374151', border: form.type === t.key ? 'none' : '1.5px solid #e5e7eb', boxShadow: form.type === t.key ? '0 4px 12px rgba(208,35,132,0.3)' : 'none' }}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '13px', background: loading ? '#e5e7eb' : 'linear-gradient(135deg,#d63384,#c026d3)', color: loading ? '#9ca3af' : 'white', border: 'none', borderRadius: 12, fontWeight: 700, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer' }}>
...
export default function HeroSection({ onBook }) {
  ...
  // Amenity capsule data — proper SVG icons
  const amenities = [
    ...
    <>
      ...
      <div className="relative w-full overflow-hidden" style={{ height: '90vh', minHeight: '560px' }}>
        ...
        <div className="absolute top-1/3 left-0 right-0 px-6 sm:px-12 lg:px-20">
          <h1 className="font-extrabold text-white leading-tight" style={{ fontSize: 'clamp(3rem, 8vw, 6rem)' }}>HK PG Boys</h1>
          <h1 className="font-extrabold leading-tight" style={{ fontSize: 'clamp(3rem, 8vw, 6rem)', color: '#f472b6' }}>Accommodation</h1>
          ...
          <div className="mt-6 flex gap-3 flex-wrap">
            <button onClick={onBook} className="px-6 py-3 rounded-xl text-white font-bold hover:opacity-90 transition" style={{ background: 'linear-gradient(135deg,#d63384,#c026d3)' }}>
              Explore Rooms →
            </button>
            <button onClick={() => setShowVisitModal(true)} className="px-6 py-3 rounded-xl font-bold transition hover:bg-white/30"
              style={{ background: 'rgba(255,255,255,0.15)', color: 'white', border: '1.5px solid rgba(255,255,255,0.4)', backdropFilter: 'blur(8px)' }}>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {amenities.map((item, i) => (
              <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-white/20 text-white backdrop-blur-md border border-white/20">
