...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    const id = showToast.loading('Signing In...', 'Verifying your credentials.')
    const result = await login(form.email, form.password)
    setLoading(false)
    if (!result.success) { showToast.update(id, 'error', 'Login Failed', result.error || 'Invalid email or password.'); return }
    showToast.update(id, 'success', result.role === 'admin' ? 'Welcome, Admin! 👑' : 'Welcome Back! 👋', 'Redirecting...')
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-10"
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Sign in to your HK PG account</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl p-7 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link to="/forgot-password" className="text-xs font-semibold hover:underline" style={{ color: '#c026d3' }}>
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} placeholder="Enter your password"
                  ...
                  className={`${inp} pr-12`} autoComplete="current-password" />
          ...
          <p className="text-center text-sm text-gray-500">
            ...
            <Link to="/register" className="font-bold hover:underline" style={{ color: '#c026d3' }}>Create account</Link>
          </p>
        </div>
